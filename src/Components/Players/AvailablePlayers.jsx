...
const AvailablePlayers = ({ playersPromise, setAvailableBalance, availableBalance, setSelectedPlayers, selectedPlayers }) => {
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 items-center justify-center gap-8'>
            {
                players.map(player => <Player player={player} setSelectedPlayers={setSelectedPlayers}
                    selectedPlayers={selectedPlayers}
                    setAvailableBalance={setAvailableBalance} availableBalance={availableBalance} key={player.id}></Player>)
            }
        </div>
    );
};
